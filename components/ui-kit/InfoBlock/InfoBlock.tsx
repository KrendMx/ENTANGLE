...
const InfoBlock: React.FC<InfoBlockProps> = ({value, type, info, ...props}: InfoBlockProps) => {
    ...
    if (value !== null) {
        switch (type) {
            ...
            case InfoBlockTypes.BALANCE: {
                ...
                additionalBottomBlock = <div className={styles.horisontalWrapper}>
                    <div className={classNames(additionalBottomBlockClass)}>{`${sign}$${Number(Math.abs(changeValue)).toFixed(4)}`}</div>
            ...
            case InfoBlockTypes.PERCENTAGE_MIXED: {
                ...
                valueText = <div className={styles.linedValue}>
                    {image && <div className={styles.smallImg}>
                        {image}
                    </div>}
                    <div>{`${sign}${value.toFixed(4)}%`}</div>
                    <div className={styles.smallText}>({signChange}${Number(Math.abs(changeValue)).toFixed(4)})</div>
                </div>;
                break;
            }
    ...
    return (
        <div className={styles.blockWrapper}>
            ...
            <div className={classNames(styles.blockValue, additionalClass)}>{additionalValuePrefixBLock}{valueText}</div>
