...
import SoonChart from '../../components/ui-kit/SoonChart/SoonChart';

import InvestCard from './InvestCard';
import Typography from '../../components/Typography';
import Select, { Option } from '../../components/ui-kit/Select';
import InfoBlock from '../../components/ui-kit/InfoBlock/InfoBlock';
import { InfoBlockTypes } from '../../components/ui-kit/InfoBlock/InfoBlock.constants';
import {
    ftmDex,
    ftmSynth,
    avaDex,
    avaSynth,
} from '../../components/HomePage/Dashboard/DashboardItem/containers/abi/index';
...
const Profile = () => {
    ...
    useEffect(() => {
        (async () => {
            if (account) {
                ...
                const avaxSynthPosition =
                    Number(avaxAccountBalance.toBigInt()) / 10 ** ftmDec;
                ...
                const synthPosition =
                    Number(accountBalance.toBigInt()) / 10 ** avaDec;
    ...
    useEffect(() => {
        if (ftmState?.positions && ftmState?.price || avaxState?.positions && avaxState?.price) {
            const ftmBalance =
                Number(ftmState?.positions!) * Number(ftmState?.price!);
            const avaxBalance =
                Number(avaxState?.positions!) * Number(avaxState?.price!);
