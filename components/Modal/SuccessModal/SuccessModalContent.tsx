...
const SuccessModalContent: React.FC<SuccessModalProps> = ({
    ...
    return <div className={styles.wrapper}>
        ...
        <div className={styles.valueTextWrapper}><span className={styles.valueText}>{transactionInfo.value}</span> {transactionInfo.symbol}</div>
        <div className={styles.btnWrapper}>
            {transactionInfo.isReceived && <div className={classNames(styles.gradientBtnWrapper, styles.btnActionWrapper, styles.flex1)}>
                <GradientButton
                    titleElement={<div className={styles.metamaskBtnWrapper}>
                        <div>Add to MetaMask</div>
                        <img className={styles.metamaskBtnImg} src="./images/connectors/metamask.svg" alt=""/></div>}
                    onClick={() => {
                        importToken()
                    }}
                    title=""/>
